...
public class DeviceService
{
    ...
    private async Task SendHeartbeatAsync()
    {
        ...
        {
            LogService.Instance.Log("Debug", "Heartbeat", "DeviceService", "未注册或未加入组织，跳过心跳");
            return;
        }
        ...
        {
            LogService.Instance.Log("Debug", "Heartbeat", "DeviceService", "组织信息不完整，跳过心跳");
            return;
        }
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                // LogService.Instance.Log("Debug", "Heartbeat", "DeviceService", $"心跳发送成功: {response.StatusCode}");
            }
            else
            {
                LogService.Instance.Log("Warning", "Heartbeat", "DeviceService", $"心跳发送失败: {response.StatusCode}");
            }
        }
        catch (TaskCanceledException ex)
        {
            LogService.Instance.Log("Error", "Heartbeat", "DeviceService", $"心跳发送超时：{ex.Message}");
        }
        catch (Exception ex)
        {
            LogService.Instance.Log("Error", "Heartbeat", "DeviceService", $"心跳发送失败：{ex.Message}");
        }
    }
    ...
    public async Task SendOfflineNotificationAsync()
    {
        if (!_isRegistered || _organizationService == null || !_organizationService.HasJoinedOrganization)
        {
            LogService.Instance.Log("Debug", "DeviceOffline", "DeviceService", "未加入组织或未注册，跳过离线通知");
            return;
        }
        ...
        if (org == null || string.IsNullOrEmpty(org.ServerUrl))
        {
            LogService.Instance.Log("Error", "DeviceOffline", "DeviceService", "组织信息不完整，无法发送离线通知");
            return;
        }

        try
        {
            LogService.Instance.Log("Info", "Device", "DeviceService", $"发送离线通知到组织：{org.Name} (服务器：{org.ServerUrl})");
            Console.WriteLine($"发送离线通知：{_deviceId} -> {org.Name}");
            ...
            var json = JsonSerializer.Serialize(offline);
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            ...
            Console.WriteLine($"离线通知发送结果：{response.StatusCode}");
            LogService.Instance.Log("Info", "DeviceOffline", "DeviceService", $"离线通知发送结果：{response.StatusCode}");
        }
        catch (OperationCanceledException)
        {
            LogService.Instance.Log("Warning", "DeviceOffline", "DeviceService", "离线通知发送超时（已忽略）");
            Console.WriteLine("离线通知发送超时（已忽略）");
        }
        catch (Exception ex)
        {
            LogService.Instance.Log("Debug", "DeviceOffline", "DeviceService", $"发送离线通知失败：{ex.Message}");
            Console.WriteLine($"发送离线通知失败：{ex.Message}");
        }
    }
