    ...
    public static LockSettingsModel Lock
    {
        get
        {
            ...
            lock (_lockObj)
            {
                if (_lock == null)
                {
                    _lock = LoadSettings<LockSettingsModel>(LockSettingsPath);
                    if (_lock.BreakTimeLockMode == (LockMode)1)
                    {
                        _lock.BreakTimeLockMode = LockMode.Full;
                        SaveLock(_lock);
                    }
                }
                return _lock;
    ...
    private static void MigrateIfNeeded()
    {
        ...
        if (File.Exists(OldLockSettingsPath) && !File.Exists(LockSettingsPath))
        {
            try
            {
                ...
                if (oldSettings.TryGetProperty("breakTimeLockMode", out var breakTimeLockMode))
                {
                    var modeValue = breakTimeLockMode.GetInt32();
