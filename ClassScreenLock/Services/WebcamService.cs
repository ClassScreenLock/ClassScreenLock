...
public class WebcamService
{
    ...
    public void CaptureOnce(string? cameraMoniker = null, bool? isClassOverride = null)
    {
        ...
        if (string.IsNullOrEmpty(moniker))
        {
            try
            {
                var cameraList = GetAvailableCameras();
                moniker = cameraList?.FirstOrDefault() ?? string.Empty;
            }
        ...
        if (string.IsNullOrEmpty(moniker))
        {
            System.Diagnostics.Debug.WriteLine("No camera available for capture");
            return;
        }
    ...
    private void TakePhoto(bool isClass, string cameraMoniker)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("Webcam capture skipped: another capture is in progress");
            return;
        }
        ...
        Task.Run(() =>
        {
            try
            {
                CaptureFromCamera(cameraMoniker, isClass);
            }
            catch (Exception ex)
            {
    ...
    private void CaptureFromCamera(string cameraMoniker, bool isClass)
    {
        ...
        try
        {
            ...
            using var frameReceived = new ManualResetEvent(false);
            ...
            void VideoSourceErrorHandler(object sender, VideoSourceErrorEventArgs eventArgs)
            {
                errorMessage = eventArgs.Description;
                frameReceived.Set();
            }

            videoSource.NewFrame += NewFrameHandler;
            videoSource.VideoSourceError += VideoSourceErrorHandler;
            videoSource.Start();

            int waitResult = WaitHandle.WaitAny(new[] { frameReceived, cts.Token.WaitHandle }, 5000);

            if (waitResult == WaitHandle.WaitTimeout)
            {
                System.Diagnostics.Debug.WriteLine("Webcam capture timed out (5 seconds)");
                LogService.Instance.Log("自动化", "拍照超时", "摄像头", "等待摄像头响应超时");
            }
            else if (waitResult == 1 || cts.Token.IsCancellationRequested)
            {
                System.Diagnostics.Debug.WriteLine("Webcam capture cancelled");
            }
            else if (errorMessage != null)
            {
                System.Diagnostics.Debug.WriteLine($"Webcam capture error: {errorMessage}");
                LogService.Instance.Log("自动化", "拍照失败", "摄像头错误", errorMessage);
            }
            ...
            if (videoSource != null)
            {
                try
                {
                    if (videoSource.IsRunning)
                    {
                        ...
                        if (videoSource.IsRunning)
                        {
                            System.Diagnostics.Debug.WriteLine("Webcam Stop timed out, forcing stop");
                            videoSource.Stop();
                        }
                    }
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Failed to stop webcam: {ex.Message}");
                }
                videoSource = null;
            }
            cts.Dispose();
